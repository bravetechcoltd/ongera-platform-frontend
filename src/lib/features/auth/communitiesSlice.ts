...
export const editCommunity = createAsyncThunk(
  "communities/edit",
  async (params: { id: string; formData: FormData }, { rejectWithValue }) => {
    try {
...
export const deleteCommunity = createAsyncThunk(
  ...
  async ({ id, reason }: { id: string; reason?: string }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || "Failed to permanently delete community")
    }
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteCommunity.fulfilled, (state, action) => {
        state.isSubmitting = false
        // Remove from all arrays (complete removal)
        state.adminCommunities = state.adminCommunities.filter(c => c.id !== action.payload.id)
        state.communities = state.communities.filter(c => c.id !== action.payload.id)
        state.myCommunities = state.myCommunities.filter(c => c.id !== action.payload.id)
        state.pendingCommunities = state.pendingCommunities.filter(c => c.id !== action.payload.id)

        // Clear current community if it was deleted
        if (state.currentCommunity?.id === action.payload.id) {
          state.currentCommunity = null
        }
      })
...
.addCase(editCommunity.fulfilled, (state, action) => {
  ...
  const index = state.communities.findIndex(c => c.id === updatedCommunity.id)
  if (index !== -1) {
  ...
  const adminIndex = state.adminCommunities.findIndex(c => c.id === updatedCommunity.id)
  if (adminIndex !== -1) {
    state.adminCommunities[adminIndex] = updatedCommunity
  }
  ...
  // ✅ REMOVED: toast.success from reducer
})
.addCase(editCommunity.rejected, (state, action) => {
  state.isSubmitting = false
  state.error = action.payload as string
  // ✅ REMOVED: toast.error from reducer
})
  }
})
