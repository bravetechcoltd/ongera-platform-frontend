...
const ensureString = (value: unknown, fallback = 'An error occurred'): string => {
  if (typeof value === 'string') return value
  if (value && typeof (value as any).message === 'string') return (value as any).message
  return fallback
}
...
export interface EnhancedUser {
  ...
  // Institution Research Portal additive fields
  institution_portal_role?: "STUDENT" | "INSTRUCTOR" | "INDUSTRIAL_SUPERVISOR" | "INSTITUTION_ADMIN" | null
  is_industrial_supervisor?: boolean
  industrial_supervisor_institutions?: string[]
  has_institution_portal_access?: boolean
...
export interface EnhancedAuthState {
  ...
  // ✅ NEW: BwengePlus Application Flow State
...
const normalizeEnhancedUser = (backendUser: any): EnhancedUser => {
  ...
  validateCrossSystemData(normalized)
...
// ✅ ENHANCED: Register User with system identification (supports Ongera + BwengePlus Apply-to-Join)
export const registerUser = createAsyncThunk(
  ...
  async (
    userData: {
      ...
      // ✅ NEW: BwengePlus Apply-to-Join fields
      IsForWhichSystem?: SystemType
      motivation?: string
      linkedin_url?: string
      country?: string
    ...
    try {
      // Default to Ongera flow unless caller specifies BwengePlus
      const enhancedUserData = {
        ...userData,
        IsForWhichSystem: userData.IsForWhichSystem || SystemType.ONGERA
      }
      ...
      if (response.data.success) {
        ...
        return {
          ...
          IsForWhichSystem: enhancedUserData.IsForWhichSystem,
...
export const loginUser = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      // ✅ NEW: Pass through application-flow error codes
      if (errorData?.code) {
        return rejectWithValue({
          message: errorData.message,
          code: errorData.code,
    ...
    builder
      ...
      .addCase(registerUser.fulfilled, (state, action) => {
        ...
        if (action.payload.application_submitted) {
          // ✅ NEW: BwengePlus Apply-to-Join — do NOT route to verify-email
          state.applicationSubmitted = true
          state.applicationEmail = action.payload.email
          state.requiresVerification = false
          state.verificationEmail = null
        } else {
          // Original Ongera flow — still requires OTP verification
          state.requiresVerification = true
      ...
      .addCase(googleLogin.rejected, (state, action: any) => {
        ...
        if (payload && typeof payload === 'object' && payload.code) {
          state.error = ensureString(payload.message, 'Google login failed')
          state.errorCode = payload.code
          state.rejectionReason = payload.rejection_reason || null
        } else {
          state.error = ensureString(payload, 'Google login failed')
        }
        state.protection.active = true
      })
      .addCase(loginUser.rejected, (state, action: any) => {
        ...
        if (payload && typeof payload === 'object') {
          if (payload.requires_verification) {
            state.error = ensureString(payload.message, 'Verification required')
            state.requiresVerification = true
            state.verificationEmail = payload.email
          } else if (payload.code) {
            state.error = ensureString(payload.message, 'Login failed')
            state.errorCode = payload.code
            state.rejectionReason = payload.rejection_reason || null
          } else {
            state.error = ensureString(payload.message || payload, 'Login failed')  // ← fixed
          }
        } else {
          state.error = ensureString(payload, 'Login failed')
        }
        state.protection.active = true
      })
...
export const selectErrorCode = (state: { auth: EnhancedAuthState }) => state.auth.errorCode
export const selectRejectionReason = (state: { auth: EnhancedAuthState }) => state.auth.rejectionReason
export const selectApplicationSubmitted = (state: { auth: EnhancedAuthState }) => state.auth.applicationSubmitted
export const selectApplicationEmail = (state: { auth: EnhancedAuthState }) => state.auth.applicationEmail
