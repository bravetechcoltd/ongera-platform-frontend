...
export default function CommunityDashboard() {
  ...
  const { currentCommunity, communityPosts, postsLoading, isLoading, isSubmitting, error } = useAppSelector((state: any) => state.communities)
  const { communityEvents, isLoading: eventsLoading } = useAppSelector((state: any) => state.events)
  const communityId = params.id as string

  // Initial load - fetch community and all data once
  useEffect(() => {
    if (communityId) {
  ...
  // Only filter posts when post type changes (no refetch)
  useEffect(() => {
                  ...
                  <div>
                    {!isMember ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          {communityProjects.slice(0, projectsToShow).map((project) => (
                            <div
                              key={project.id}
                        ...
                        {communityProjects.length > projectsToShow && (
                          <div className="flex justify-center mt-6">
                            <button
