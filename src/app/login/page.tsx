...
export default function EnhancedLoginPage() {
  ...
  const [enableQuickAccess, setEnableQuickAccess] = useState(true) // ✅ NEW
...
useEffect(() => {
  ...
  useEffect(() => {
    if (isAuthenticated && user && !showSSOInitializer) {
      // Redirect to dashboard if SSO not needed
...
useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ NEW: Check if SSO should be initialized
      if (result.user.sso_redirect_token && enableQuickAccess) {
        // Enable BwengePlus for future logins
        dispatch(enableBwengePlus())

        // Show SSO initializer
              ...
              {user?.has_multi_system_access && (
                <div className="mt-4 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-xs text-blue-700 text-center">
                    ✓ Connected to Ongera & BwengePlus
                  </p>
                </div>
              )}
            </div>
