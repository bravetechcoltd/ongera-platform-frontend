...
export default function SSOInitializer({ ssoToken, onComplete, onSkip }: SSOInitializerProps) {
  ...
  const [message, setMessage] = useState('Preparing access to Bwenge...');
  const [countdown, setCountdown] = useState(3);

  useEffect(() => {
    const initializeSSO = async () => {
      try {
        setMessage('Connecting to Bwenge...');
        
        // Create hidden iframe to silently authenticate with Bwenge
        ...
        setStatus('success');
        setMessage('✓ Bwenge access enabled!');
        ...
        setStatus('error');
        setMessage('Failed to connect to Bwenge');
      }
    };
