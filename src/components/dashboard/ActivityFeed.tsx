...
export default function ActivityFeed({ recentActivity, compact = true, limit = 5 }: ActivityFeedProps) {
  ...
  const getActivityIcon = (type: string) => {
    const iconConfig = {
      project: { icon: Upload, color: 'bg-gradient-to-br from-emerald-500 to-emerald-600', bg: 'bg-emerald-50', text: 'text-emerald-600' },
      post: { icon: MessageSquare, color: 'bg-gradient-to-br from-blue-500 to-blue-600', bg: 'bg-blue-50', text: 'text-blue-600' },
      event: { icon: Calendar, color: 'bg-gradient-to-br from-purple-500 to-purple-600', bg: 'bg-purple-50', text: 'text-purple-600' },
      comment: { icon: MessageSquare, color: 'bg-gradient-to-br from-amber-500 to-amber-600', bg: 'bg-amber-50', text: 'text-amber-600' },
      like: { icon: Heart, color: 'bg-gradient-to-br from-rose-500 to-rose-600', bg: 'bg-rose-50', text: 'text-rose-600' },
      follow: { icon: Users, color: 'bg-gradient-to-br from-indigo-500 to-indigo-600', bg: 'bg-indigo-50', text: 'text-indigo-600' },
      achievement: { icon: Award, color: 'bg-gradient-to-br from-yellow-500 to-yellow-600', bg: 'bg-yellow-50', text: 'text-yellow-600' }
    }
  ...
  const ActivityItemComponent = ({ activity, index }: { activity: typeof allActivities[0]; index: number }) => {
    ...
    return (
      <Link
        ...
        href={content.link}
        className={`flex items-start space-x-3 p-2.5 rounded-lg hover:bg-gray-50 transition-all border border-transparent hover:border-gray-200 hover:shadow-sm group ${
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              <p className={`font-medium text-gray-900 group-hover:text-[#0158B7] transition-colors ${
      ...
      <div className={compact ? 'space-y-1.5' : 'space-y-3'}>
        {allActivities.length === 0 ? (
          ...
          allActivities.map((activity, index) => (
            <ActivityItemComponent key={index} activity={activity} index={index} />
          ))
        )}
      </div>
      ...
      {compact && allActivities.length > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <Link
            ...
            className="text-sm font-medium text-[#0158B7] hover:text-[#034EA2] flex items-center justify-center gap-2 transition-colors group"
          >
            View All Activity
